    ...
    *,
    ...
    for item in items or []:
        ...
        should_consume_on_this_line = fulfillment_mode == 'make_to_order' and (
            raw_material_consumption_mode == 'fulfillment' or manual_auto_consume
        )
        if should_consume_on_this_line:
            consumption_notes = (
                f'Consumo por cumplimiento de venta #{sale.id}'
                if raw_material_consumption_mode == 'fulfillment'
                else f'Consumo automático por venta #{sale.id}'
            )
