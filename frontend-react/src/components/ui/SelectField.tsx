...
export const SelectField: React.FC<SelectFieldProps> = ({
  ...
  const mobilePanel = !isDesktop && isOpen
    ? createPortal(
        <div className="fixed inset-0 z-[9999] flex items-end justify-center p-0 pb-[env(safe-area-inset-bottom)] sm:items-center sm:p-6">
          ...
          />
          <div
            className="app-surface relative flex w-full max-h-[calc(100dvh-0.5rem)] flex-col overflow-hidden rounded-t-[28px] shadow-[var(--app-shadow-strong)] sm:max-h-[70vh] sm:max-w-md sm:rounded-[28px]"
            role="listbox"
          ...
          >
            <div className="app-page-header sticky top-0 z-10 flex items-center justify-between border-b app-divider px-5 py-4">
              <h3 id={`${selectId}-sheet-title`} className="min-w-0 truncate text-base font-semibold app-text">
                {sheetTitle || label || placeholder || 'Seleccionar'}
              </h3>
              <button
                type="button"
                onClick={() => setIsOpen(false)}
  ...
  return (
    <div className={cn('flex flex-col gap-1.5', wrapperClassName)}>
      {label ? (
        <label htmlFor={selectId} className="text-[13px] font-semibold tracking-tight app-text-secondary">
          {label}
        </label>
      ) : null}
