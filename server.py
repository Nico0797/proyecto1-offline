...
# ---------------- API: Insights (amigable) ----------------
@app.get("/api/insights")
def get_insights():
    """
    Devuelve 'insights' para dueños de negocio en lenguaje simple:
    - producto_estrella (por ingresos)
    - riesgo_cartera (qué tanto estás vendiendo fiado)
    - proyeccion_30d (si sigues igual)
    - recordatorios (a quién cobrar)
    """
    date_from = request.args.get("from")
    date_to = request.args.get("to")
    ...
    # Si no mandan rango: usamos mes actual hasta hoy
    today_local = datetime.utcnow().date()  # simple; frontend maneja zona
    if not date_to:
        date_to = today_local.strftime("%Y-%m-%d")
    if not date_from:
        date_from = date_to[:7] + "-01"

    conn = db()

    # 1) Producto estrella en el rango
    star = conn.execute("""
    ...
    product_star = None
    if star and float(star["revenue"]) > 0:
        product_star = {
            "product_id": star["product_id"],
            "name": star["product_name"],
            "revenue": float(star["revenue"]),
            "qty": int(star["qty"]),
            "message": f"Tu producto estrella en este rango fue **{star['product_name']}**."
        }

    # 2) Riesgo cartera (cuánto hay por cobrar vs ventas del rango)
    sales_total_row = conn.execute("""
        SELECT COALESCE(SUM(total),0) AS total
        FROM sales
        WHERE sale_date BETWEEN ? AND ?
    """, (date_from, date_to)).fetchone()
    sales_total = float(sales_total_row["total"])
    ...
    ledger_row = conn.execute("""
        ...
        FROM ledger
    """).fetchone()
    outstanding = float(ledger_row["charges"]) - float(ledger_row["payments"])
    if outstanding < 0:
        outstanding = 0.0

    denom = sales_total if sales_total > 0 else 1.0
    ratio = outstanding / denom

    if ratio <= 0.10:
        level = "tranqui"
        tip = "Vas bien: casi todo está entrando completo."
    elif ratio <= 0.25:
        level = "ojo"
        tip = "Ojo: estás fiando una parte importante. Ponle reglas a los fiados."
    else:
        level = "alerta"
        tip = "Alerta: mucha plata está en la calle. Enfócate en cobrar y baja fiados."
    ...
    # 3) Proyección 30 días (promedio últimos 30 días)
    end_dt = datetime.strptime(date_to, "%Y-%m-%d").date()
    start_30 = (end_dt - dt.timedelta(days=29)).strftime("%Y-%m-%d")
    last30_row = conn.execute("""
        SELECT COALESCE(SUM(total),0) AS total
        FROM sales
        WHERE sale_date BETWEEN ? AND ?
    """, (start_30, date_to)).fetchone()
    last30_total = float(last30_row["total"])
    avg_daily = last30_total / 30.0
    projected_30 = avg_daily * 30.0

    projection = {
        "range": {"from": start_30, "to": date_to},
        "last30_total": round(last30_total, 2),
        "avg_daily": round(avg_daily, 2),
        "projected_30": round(projected_30, 2),
        "message": "Si sigues con el mismo ritmo de los últimos 30 días, esto es lo que pinta para el próximo mes."
    }

    # 4) Recordatorios: top clientes con saldo + fecha más vieja sin pagar
    reminders_rows = conn.execute("""
        WITH charge_due AS (
          SELECT l.id, l.customer_id, l.entry_date,
                 (l.amount - COALESCE(SUM(a.amount),0)) AS due
          FROM ledger l
          LEFT JOIN ledger_allocations a ON a.charge_id = l.id
        ...
        SELECT
          ...
          c.name AS customer_name,
          ROUND(
            COALESCE(SUM(CASE WHEN l.entry_type='charge' THEN l.amount END),0) -
            COALESCE(SUM(CASE WHEN l.entry_type='payment' THEN l.amount END),0)
          , 2) AS balance,
          MIN(cd.entry_date) AS oldest_due_date
        FROM customers c
        LEFT JOIN ledger l ON l.customer_id=c.id
        LEFT JOIN charge_due cd ON cd.customer_id=c.id
        GROUP BY c.id
    ...
    for r in reminders_rows:
        ...
        if oldest:
            try:
                od = datetime.strptime(oldest, "%Y-%m-%d").date()
                days = (end_dt - od).days
            except:
                days = None
        reminders.append({
            ...
            "days_overdue": days,
            "message": f"A {r['customer_name']} le faltan {round(float(r['balance']),2)} por pagar."
        })
